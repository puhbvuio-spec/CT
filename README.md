...
一个 PyQt 桌面工具台，用于集中启动 YouTube、TikTok、X/Twitter、Instagram、Facebook、Twitch、Steam/SteamDB 等平台的数据采集工具，并提供 AIGC 标题判断、关键词 XLSX 合并等数据处理功能。

## 环境要求

- Python 3.10+，建议使用 3.11 或 3.12。
- Windows + Chrome 或 Chromium。
- TikTok、X/Twitter、Instagram、Facebook 以及 SteamDB/SullyGnome 动态窗口工具依赖 Playwright 接管浏览器。
- YouTube 工具需要 Google API Key。
- Twitch 工具需要 Twitch Developer Console 中的 Client ID 和 Client Secret，也可通过 `TWITCH_CLIENT_ID`、`TWITCH_CLIENT_SECRET` 环境变量提供。
- Steam API 游戏数据采集可在无 Key 时采集商店公开数据；Steam 玩家画像补采和玩家增强能力需要 Steam Web API Key。
- SteamDB 动态窗口工具使用本机 Chrome/Edge 打开可见页面，不绕过浏览器检查或站点保护；遇到检查/阻止页面时会暂停并提示用户手动处理。
...
各平台（TikTok、X/Twitter、Instagram、Facebook、SteamDB、SullyGnome 补采等）动态窗口工具会自动使用项目根目录下的 `user_data/` 启动 Chrome/Edge 调试浏览器。首次使用需要在自动打开的浏览器里登录或完成必要的人机验证。登录态会保存在 `user_data/`，后续通常不用重复登录。
...
- `src/platforms/twitch/`：Twitch Helix API 采集工具，以及 SullyGnome 动态窗口补采能力。
- `src/platforms/steam/`：Steam API、Steam 玩家画像补采和 SteamDB 动态窗口采集工具。
...
Twitch 工具基于官方 Twitch Helix API，需要填写 `Client ID` 和 `Client Secret`。密钥只保存在本机配置或环境变量中，不应提交到仓库。

### Twitch 数据采集

Twitch 数据采集模块包含游戏内容采集和 KOL 发现两个子能力。`Twitch KOL 发现` 属于 Twitch 数据采集模块下的子功能，不作为与平台级模块平级的独立章节。

#### Twitch 游戏内容采集

用途：按 Twitch 游戏名或 Game ID 采集游戏相关内容，并可补充全平台 Top Games 数据。

输入：
- Twitch Client ID
- Twitch Client Secret
- 游戏名 / Game ID：每行一个，可直接输入或选择 TXT 文件
- 是否采集当前直播
- 是否采集 VOD 回放
- 是否采集 Clips 片段
- 是否采集 Top Games
- 是否启用 SullyGnome 低频补采

可通过「参数配置」调整：每个游戏最多直播数、每个游戏最多 VOD 数、每个游戏最多 Clips 数、Clips 时间窗口、Top Games 页数、请求等待、缓存时长，以及 SullyGnome 摘要范围、是否采集可见表格、可见表格行数、最大滚动次数、页面超时和浏览器类型。

输出工作表包括：`游戏信息`、`当前直播`、`VOD回放`、`Clips片段`、`TopGames`。启用 SullyGnome 后会额外写入 `SullyGnome摘要` 和 `SullyGnome可见表`。

#### Twitch KOL 发现

用途：按关键词搜索 Twitch 频道，并通过热门 VOD 挖掘创作者，补采关注者、创作者等级、注册时间和简介后输出 KOL 评分表。
...
可通过「参数配置」调整：每个关键词最多频道数、每个关键词最多 VOD 数、最小关注者阈值、请求等待、缓存时长和排序/评分相关参数。

输出工作表包括：`KOL推荐`、`主播画像`、`命中VOD`、`数据说明`。

#### SullyGnome 补充说明

SullyGnome 不是独立工具，而是 `Twitch 游戏内容采集` 中的可选补采项。它通过 Playwright 接管本机 Chrome/Edge，低频打开 SullyGnome 的可见页面，提取游戏摘要和当前可见表格行。该能力只读取浏览器中正常展示的页面内容，不做站点保护绕过。

## Steam 工具

Steam 工具分为官方/公开接口采集和 SteamDB 动态窗口采集两类。Steam API 工具会优先使用 Steam Store/Public API；玩家画像相关能力需要 Steam Web API Key。SteamDB 工具使用本机浏览器动态窗口读取可见页面数据。

### Steam API 游戏数据采集

用途：按 Steam AppID、商店链接、SteamDB 链接或关键词发现游戏，并采集游戏商店信息、评价摘要、玩家评论、新闻、当前在线人数和成就数量；可选基于评论玩家继续补采玩家画像。

输入：
- Steam Web API Key（可选；用于 IStoreService AppList 和玩家增强能力）
- AppID / Steam 商店链接：每行一个
- 关键词：每行一个（可选，用 Steam 商店公开接口发现 AppID）
- 语言和商店地区代码
- 是否采集玩家评论/评价摘要
- 每个游戏最多玩家评论数
- 是否采集新闻
- 是否采集当前在线人数、成就数量、评论玩家画像

可通过「参数配置」调整：关键词发现数量、每个关键词最多 App 数、评论语言/排序/过滤方式、请求超时、请求间隔、缓存时长、并发数、每批保存条数，以及玩家画像增强相关采集上限。

输出工作表包括：`游戏信息`、`玩家评论`、`新闻`，开启玩家画像增强后还会写入 `玩家画像`、`玩家游戏库`、`最近游玩`、`目标游戏成就`、`玩家徽章`、`玩家好友`。

### Steam 评论玩家画像补采

用途：读取已有 Steam 玩家评论 Excel，或手动输入 SteamID / Steam 个人主页链接，补采玩家公开资料、公开游戏库、公开好友、最近游玩、目标游戏成就和徽章，用于构建游戏玩家画像。

输入：
- Steam Web API Key（必填）
- 已有玩家评论 Excel（可选，优先读取 `玩家评论` 工作表）
- SteamID / 个人主页链接：每行一个（可选）
- 目标游戏 AppID / 商店链接（手填 SteamID 时可选）
- 目标游戏名（可选）

可通过「参数配置」调整：每个玩家最多游戏库条数、好友数、最近游玩条数、徽章数、目标游戏成就采集、并发数、请求间隔、缓存时长和每批保存条数。

输出工作表包括：`玩家画像`、`玩家游戏库`、`最近游玩`、`目标游戏成就`、`玩家徽章`、`玩家好友`。

### SteamDB 动态窗口采集

用途：通过本机 Chrome/Edge 动态窗口采集 SteamDB App 页面中可见的结构化数据，首批支持概览页、Charts 页、Packages/DLCs、Depots 和 History。

输入：
- AppID / SteamDB 链接 / Steam 商店链接：每行一个
- 关键词：每行一个（可选，使用 Steam 商店公开接口发现 AppID）
- 已有 Excel（可选，读取 AppID 列）
- 关键词发现语言和地区代码

可通过「参数配置」调整：浏览器类型（Chrome/Edge）、要采集的页面类型、遇到浏览器检查时的处理方式、最多 App 数、每个关键词最多 App 数、是否包含非游戏、页面超时、加载等待、页面间隔、最大滚动次数、每页最多可见表格行数、请求缓存和每批保存条数。

输出工作表包括：`采集状态`、`概览`、`Charts`、`Packages`、`DLCs`、`Depots`、`History`、`RawTables`。

说明：
- 该工具不会使用原始 HTTP 请求抓取 SteamDB 页面，也不会绕过 Cloudflare/浏览器检查或 SteamDB 保护。
- 如果页面出现浏览器检查、Access Denied 或阻止提示，任务会暂停并在日志中提示。请在打开的浏览器中手动完成检查或确认页面状态，然后点击「继续」恢复。
- 工具只访问公开页面，不构造被明确禁用的查询形态，例如 `/app/*/depots/?branch=*` 或 `/app/*/history/?changeid=*`。
...
twitch_game_content_YYYYMMDD_HHMMSS.xlsx
twitch_kol_discovery_YYYYMMDD_HHMMSS.xlsx
steam_api_research_YYYYMMDD_HHMMSS.xlsx
steam_player_profiles_YYYYMMDD_HHMMSS.xlsx
steamdb_dynamic_window_YYYYMMDD_HHMMSS.xlsx
```

启用分级输出时，文件会按平台、工具通道和日期进一步归档，例如 `output/steam/steam_api_research/20260630/steam_api_research_20260630_120000.xlsx`。

## 运行建议

- 首次使用 TikTok、X/Twitter、Instagram 或 Facebook 前，先运行任意对应平台工具，让程序打开浏览器，然后完成登录。
- Twitch 工具需要有效的 Twitch Client ID/Secret；如果 UI 未填写，会尝试读取 `TWITCH_CLIENT_ID` 和 `TWITCH_CLIENT_SECRET` 环境变量。
- Steam API 游戏数据采集不强制要求 Steam Web API Key，但玩家画像、公开游戏库、好友、成就、徽章等玩家增强能力需要有效 Key。
- SteamDB 动态窗口采集前建议先用同一个浏览器手动打开 SteamDB，确认页面可访问；遇到浏览器检查时按日志提示在浏览器中处理后再继续。
- 每个工具窗口提供「参数配置」按钮，可在启动任务前调整爬取行为参数（超时、等待间隔、停止阈值等）。修改后的参数会保留到下次打开。
- 任务运行中可点击「暂停」暂停采集并保留当前进度，点击「继续」从暂停点恢复；点击「停止」彻底终止任务。
- 不要频繁并发运行多个 X/Twitter、TikTok、SteamDB 或 SullyGnome 动态窗口任务，容易触发平台限制或浏览器检查。
...
优先检查：
- Twitch Client ID 和 Client Secret 是否来自 Twitch Developer Console。
- Client Secret 是否完整复制，未包含多余空格。
- 如果依赖环境变量，确认 `TWITCH_CLIENT_ID` 和 `TWITCH_CLIENT_SECRET` 已在当前启动环境中设置。
- Twitch Helix API 是否临时限流或网络不可达。

### Steam 玩家画像为空

优先检查：
- Steam Web API Key 是否有效。
- 目标玩家资料、游戏库、好友或成就是否公开。
- 输入的 SteamID / 个人主页链接是否能解析为 `7656...` 格式 SteamID。
- 从 Excel 补采时，确认文件中存在可识别的 SteamID 或玩家主页列。

### SteamDB 出现浏览器检查或 Access Denied

处理方式：
- 不要尝试绕过站点保护。
- 在工具打开的 Chrome/Edge 页面中手动完成检查，或确认当前网络/浏览器会话可以正常访问 SteamDB。
- 完成后回到工具窗口点击「继续」。如果按钮状态未同步，可按日志提示切换一次暂停/继续。
- 如果页面长期不可访问，降低采集频率、减少并发和批量规模后再重试。
