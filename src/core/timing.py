...
def interruptible_sleep(seconds: float, stop_event=None, step: float = 0.2, pause_event=None) -> bool:
    """
    ...
    while time.time() < end_time:
        ...
        if pause_event and pause_event.is_set():
            ...
            end_time += time.time() - paused_at
        # 睡剩余时长和步长之间的较小值，避免睡超时间或者因单次 sleep 过长导致响应卡顿
        time.sleep(min(step, max(0, end_time - time.time())))
    return should_stop(stop_event)


def random_cooldown(log_callback=None, stop_event=None, min_seconds: float = 3.0, max_seconds: float = 8.0, reason: str = "降低访问频率", pause_event=None):
    """
...
def interruptible_random_sleep(min_seconds: float, max_seconds: float, log_callback=None, stop_event=None, reason: str = "降低访问频率", pause_event=None):
    """
