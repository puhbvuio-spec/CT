...
class SteamDbDynamicWindow(SimpleToolWindow):
    ...
    def tool_config_params(self):
        return [
            ...
            ConfigParam("steamdb_download_visible_data", "尝试可见下载按钮", kind="combo", options=("否", "是"), default="否", tooltip="只点击页面上已经可见的下载/导出控件；没有则回退表格数据。"),
            ConfigParam("steamdb_download_max_per_page", "每页最多下载控件", kind="int", default=1, minimum=1, maximum=10),
    ...
    def run_task(self, values, log_callback, finish_callback, stop_event, pause_event):
        ...
        config_keys = {
            ...
            "collect_history",
            "block_handling",
            "steamdb_download_visible_data",
            "steamdb_download_max_per_page",
            "max_apps",
